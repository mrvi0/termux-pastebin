...
### Added
- **Приватные пасты:**
    - Добавлена возможность создавать приватные пасты (доступные только автору) с помощью чекбокса на главной странице.
    - Реализована проверка прав доступа при просмотре паст: приватные пасты доступны только авторизованному автору.
- **Шифрование приватных паст:**
    - Содержимое приватных паст теперь шифруется в базе данных (AES-GCM) перед сохранением.
    - Дешифрование происходит при просмотре приватной пасты автором.
    - Добавлен модуль `src/security.py` для функций шифрования/дешифрования.
    - Требуется переменная окружения `PASTE_ENCRYPTION_KEY` (base64, 32 байта) в `.env`.
- Отображение статуса (Публичная/Приватная) на странице "Мои пасты" и на странице просмотра пасты.
- Отображение дешифрованного превью для приватных паст на странице "Мои пасты".

### Changed
- Обновлена структура таблицы `pastes` в БД (добавлена колонка `is_public`).
- Обновлены функции `add_paste`, `get_paste`, `get_user_pastes` в `src/database.py` для поддержки приватности и шифрования.
- Обновлен роут `/` (POST) для сохранения статуса `is_public`.
- Обновлен роут `/<paste_key>` для проверки прав доступа к приватным пастам.
- Обновлены шаблоны `home.html`, `my_pastes.html`, `view_paste.html`.
- Добавлена зависимость `cryptography` (если еще не была добавлена через `Authlib`).

### Fixed
- Исправлено отображение `created_at` в шаблоне `my_pastes.html` (обработка `None` или не-datetime).
