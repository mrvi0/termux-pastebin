...
def convert_timestamp(val):
    try:
        ...
        try:
            ...
            logger.warning(
                f"Не удалось преобразовать timestamp '{val}' ни одним из способов."
            )
            return None
...
def add_paste(
    ...
    conn: sqlite3.Connection | None = None
    logger.debug(
        f"Вызов add_paste для key={paste_key}, user_id={user_id}, public={is_public}"
    )
    ...
    if not is_public:
        ...
        if encrypted_content is None:
            logger.error(
                f"ШИФРОВАНИЕ НЕ УДАЛОСЬ для пасты {paste_key}! Паста не будет добавлена."
            )
            return None
        else:
            ...
            encrypted = True
            logger.info(
                f"Паста {paste_key} УСПЕШНО зашифрована ({len(content_to_save)} байт). Тип: {type(content_to_save)}"
            )
    else:
        logger.debug(f"Паста {paste_key} публичная, шифрование не требуется.")

    logger.debug(
        f"Данные для записи в БД (key={paste_key}, encrypted={encrypted}): тип={type(content_to_save)}"
    )
    ...
    try:
        ...
        conn.commit()
        logger.info(
            f"Паста {paste_key} успешно записана в БД (user_id={user_id}, public={is_public}, encrypted={encrypted})."
        )
        return paste_key
...
def get_paste(paste_key: str) -> tuple[str, str | None, int | None, bool] | None:
    ...
    try:
        ...
        cursor.execute(
            "SELECT content, language, user_id, is_public, created_at FROM pastes WHERE key = ?",
            (paste_key,),
        )  # Добавил created_at для лога
        row = cursor.fetchone()
        ...
        if row:
            ...
            logger.debug(
                f"Паста '{paste_key}' найдена в БД. is_public={is_public_flag}, тип контента={type(content_from_db)}, создана={created_at}"
            )
            ...
            if not is_public_flag:
                ...
                if isinstance(content_from_db, bytes):
                    ...
                    logger.error(
                        f"Приватная паста {paste_key} хранится не как BLOB! Тип: {type(content_from_db)}"
                    )
            ...
            logger.info(
                f"Возврат данных для пасты '{paste_key}' (user_id={author_user_id}, public={is_public_flag})."
            )
...
def get_user_pastes(
    user_id: int, limit: int = 50, preview_len: int = 150
) -> list[dict[str, Any]]:
    """
            ...
            if is_public:
                if isinstance(content_from_db, str):
                    ...
                    try:
                        ...
                        )  # Заменяем ошибки декодирования
                    except Exception:
                        final_content_preview = "[Ошибка декодирования]"
                else:
                    final_content_preview = "[Неизвестный тип]"
            else:  # Приватная паста
                if isinstance(content_from_db, bytes):
                    logger.debug(
                        f"Дешифровка для превью (my_pastes) key={key}. Тип: {type(content_from_db)}"
                    )
                    decrypted_content = security.decrypt_content(content_from_db)
                    if decrypted_content is not None:
                        final_content_preview = (
                            decrypted_content  # Возвращаем ПОЛНЫЙ дешифрованный текст
                        )
                    ...
                    logger.error(
                        f"Приватная паста {key} хранится не как BLOB для превью! Тип: {type(content_from_db)}"
                    )

            # Записываем обработанный (или дешифрованный) контент
            paste_dict["content"] = (
                final_content_preview
                if final_content_preview is not None
