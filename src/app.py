...
try:
    file_handler = logging.handlers.RotatingFileHandler(
        log_file_path, maxBytes=5*1024*1024, backupCount=3, encoding='utf-8' # 5MB, 3 копии
    )
...
logging.getLogger('werkzeug').setLevel(logging.WARNING) # Логи запросов Flask будут на уровне INFO/DEBUG нашего логгера
...
logger = app.logger # Можно продолжать использовать app.logger, он теперь настроен через root

# --- Конфигурация Flask (Secret Key) ---
app.config['SECRET_KEY'] = os.environ.get("FLASK_SECRET_KEY")
if not app.config['SECRET_KEY']:
    logger.critical("FLASK_SECRET_KEY не установлен! Работа flash и сессий НЕ ГАРАНТИРОВАНА!")
...
if not yandex_client_id or not yandex_client_secret:
    logger.error("YANDEX_CLIENT_ID или YANDEX_CLIENT_SECRET не установлены! Авторизация Яндекс не будет работать.")
else:
    oauth.register(
        name='yandex',
...
def authorize_yandex():
    ...
    try:
        ...
        logger.info(f"Получена информация о пользователе Яндекс ID: {yandex_id}, Login: {login}")
        ...
        if user_id:
            ...
            session['display_name'] = user_info.get('display_name') or login # Используем логин, если нет display_name
            flash('Вход через Яндекс выполнен успешно!', 'success')
            logger.info(f"Пользователь Yandex ID {yandex_id} ({login}) успешно авторизован как локальный user_id {user_id}")
            ...
            logger.error(f"Не удалось найти или создать пользователя в БД для Yandex ID: {yandex_id}")
            return redirect(url_for('login'))

    except Exception as e:
        # Ловим любые другие ошибки (сетевые, OAuth и т.д.)
        logger.error(f"Общая ошибка во время callback от Яндекса: {e}", exc_info=True)
        flash(f"Произошла ошибка авторизации через Яндекс: {type(e).__name__}", 'danger')
        return redirect(url_for('login'))
...
def create_paste():
    ...
    paste_key = database.add_paste(content, user_id=user_id, language=language, is_public=is_public)

    if paste_key:
        paste_url = url_for('view_paste', paste_key=paste_key, _external=True)
        logger.info(f"Паста {paste_key} создана (user_id={user_id}, public={is_public}). URL: {paste_url}")
        flash(f"Паста создана! Ссылка: {paste_url}", "success")
        return redirect(url_for('view_paste', paste_key=paste_key))
    else:
        logger.error(f"Не удалось создать пасту для '{user_log_id}'. Ошибка БД или шифрования.")
...
def my_pastes():
    ...
    user_pastes = database.get_user_pastes(user_id) # Функция возвращает список словарей с превью
...
def view_paste(paste_key: str):
    ...
    if not re.fullmatch(r'[a-zA-Z0-9]{5,12}', paste_key):
         ...
         return render_template('view_paste.html', paste_key=paste_key, error_message="Неверный формат ключа пасты.", content=None), 404

    result = database.get_paste(paste_key) # Возвращает (content, lang, author_id, is_public)
    ...
    if result:
        ...
        if not is_public and current_user_id != author_user_id:
            logger.warning(f"Доступ запрещен к приватной пасте '{paste_key}' (user: {current_user_id}, author: {author_user_id}).")
            return render_template('view_paste.html', paste_key=paste_key, error_message="Это приватная паста. Доступ есть только у автора.", content=None), 403
        ...
        return render_template(
            'view_paste.html', paste_key=paste_key, content=content,
            language=language, is_public=is_public, author_name=author_name
        )
    else:
        logger.warning(f"Паста '{paste_key}' не найдена в БД.")
        return render_template('view_paste.html', paste_key=paste_key, error_message=f"Паста с ключом '{paste_key}' не найдена.", content=None), 404
...
def handle_exception(e: Exception):
    ...
    if isinstance(e, HTTPException):
        ...
        if e.code >= 500 or e.code not in [401, 403, 404]:
             logger.error(f"HTTP ошибка {e.code}: {e.name} для {request.url}", exc_info=True)
        else:
