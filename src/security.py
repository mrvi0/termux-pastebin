...
def _load_encryption_key() -> bytes | None:
    ...
    if not encoded_key:
        logger.critical(
            f"Переменная окружения {PASTE_ENCRYPTION_KEY_ENV} не установлена! Шифрование/дешифрование НЕВОЗМОЖНО."
        )
        return None
    try:
        ...
        if len(decoded_key) != 32:
            logger.critical(
                f"{PASTE_ENCRYPTION_KEY_ENV} должен быть длиной 32 байта после декодирования base64 (получено: {len(decoded_key)} байт)."
            )
            return None
        ...
        logger.critical(
            f"Ошибка декодирования {PASTE_ENCRYPTION_KEY_ENV} из base64: {e}"
        )
        return None
...
def encrypt_content(content: str) -> bytes | None:
    ...
    try:
        ...
        logger.debug(
            f"Контент успешно зашифрован (nonce: {len(nonce)}B, ctext+tag: {len(ciphertext_and_tag)}B, total: {len(encrypted_data)}B)."
        )
...
def decrypt_content(encrypted_data: bytes) -> str | None:
    ...
    if not isinstance(encrypted_data, bytes) or len(encrypted_data) < nonce_len:
        logger.error(
            f"Дешифрование невозможно: неверный формат входных данных (тип: {type(encrypted_data)}, длина: {len(encrypted_data) if encrypted_data else 0}). Ожидались байты длиной >= {nonce_len}."
        )
        return None
    ...
    # Ловим специфичные ошибки и общие ошибки
        ...
        logger.error(
            "Ошибка дешифрования: Неверный тег аутентификации (возможно, неверный ключ или данные повреждены).",
            exc_info=False,
        )  # Не пишем полный трейсбек для этой частой ошибки
        return None
